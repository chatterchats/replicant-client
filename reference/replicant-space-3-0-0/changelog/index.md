...
crawled_at: "2026-09-02T20:03:40.015483+00:00"
---
...
1. v3.0.0 alteration
   ...
   Welcome to Season Three - Alteration Protocol.

   Previously on Replicant Space: Riker managed to lift 10 million humans into the Ark, and Bill found a pair of wormhole gates!

   Coming up in this season: The Exodus Ark journeys to twelve systems to distribute humanity across the galaxy, while the wormhole gate studies continue over in Delta. Join the Ark team to help Ledger, Shepherd and Dash build space stations at each stop. Head to the Delta system to help Kepler and Patch establish a new research lab.

   Oh, and terraforming is a thing now.

   ### Event Stream

   - Added new Terraforming Updates section to the [Event Stream](https://stream.replicant.space/) for a live terraforming dashboard.

   ### Online docs

   - New section and pages to explain [Terraforming](../api/locations/terraforming/index.md) concepts with examples.
   - Added Intentional Impacts section to the [Asteroids](../api/locations/asteroids/index.md) page.
   - Updated the [NPCs](../concepts/npcs/index.md) page to list the new characters in the game.

   ### API updates

   - New mining.relocated event for when mining drones shift between resource sites when a resource depletes.
   - Added environmental preferences to the /species response.
   - Fixed bug with the habitable zone formula on new starter private regions.
   - Planets and moons have been updated with a different set of fields to describe them.
   - New settings field in the configuration block on PATCH /devices/:code requests, for device-specific settings.
   - New location event at SOL-3: Riker’s Warming.
   - New NPCs at the Ark - Ledger, Shepherd and Dash.
   - New NPCs at the Delta wormhole gate - Kepler and Patch.
   - New shop in the Ark’s Oort cloud for Dash.
   - New set of location events in the Delta region.
   - A few more stars.

   Remember when we used to hate asteroids? o7
2. v2.5.2 restoration

   23 August 2026

   The big thing in today’s patch is improved mining drone efficiency, under AMI control.

   Mining drones can sometimes sit idle while waiting for the next AMI evaluation cycle. This update introduces reactive re-evaluation: when a survey drone unlocks a new site, or when a mining drone depletes a resource, they will poke any AMI Mining Controllers at the location to reprocess their directives.

   Also, the changelog page on the website has been overhauled with a new interesting design. It was getting a bit long.

   ### Online docs

   - Redesigned the [Changelog](index.md) page with collapsable entries, a version sidebar, and full-text search.
   - Updated the [Event Catalogue](../api/events/catalogue/index.md) with new fields for print.completed and system.object_detected.

   ### API updates

   - AMI controllers now trigger re-evaluation on resource depletion and new site discovery events.
   - Enhanced the account wipe process to cover recently added features (wards, trading, simulations, etc).
   - Custom webhook urls that fail will now be ignored for 10 minutes.
   - Compacted devices no longer lose operational capacity.
   - The print.completed event now includes a compacted flag.
   - The system.object_detected event now includes an impact_eta value.
   - Multiple beacons at a location will no longer trigger multiple event discovery notifications.
   - Improved the BobNet NPC background chatter system to be a little more interesting.

   1.5 billion resources mined. Impressive! o7
3. v2.5.1 restoration

   21 August 2026

   This update is mostly small bug fixes and improvements as the season draws to a close.

   Everything slowed for a bit while players took the long, lonely journey across the void to discover Delta. Thousands of devices have been in flight to bootstrap in the new region.

   Riker is finalising the Exodus Ark’s travel route and disengaging from human politics. His focus is shifting to terraforming while his clones escort the Ark onwards.

   Bill is returning to his Skunkworks, while his clones head to Delta to start a new research project.

   ### Online docs

   - Added a note on vessel self-repair to the [Maintenance Drone](../drones/maintenance/index.md) page.
   - Added hub.maintained, hub.warning and multiplayer events to the [Event Catalogue](../api/events/catalogue/index.md) page.
   - New filters for `tags` and `exclude_tags` added to the [Device List](../api/devices/list/index.md) page.
   - Updated the [Locations](../api/locations/index.md) page to reflect the normalised `atmosphere` field.
   - Updated the [Achievements](https://replicant.space/achievements/) page with Ring Architect, Delta Pioneer and Arms Dealer.

   ### API updates

   - Vessel self-repair: damaged vessels with the cradle and print features now passively regenerate capacity at 1% per hour. This allows stranded vessels to slowly fix themselves.
   - New event: `hub.maintained` fires when hub repair consumes resources from the location inventory.
   - New event: `hub.warning` fires when a hub’s capacity drops below a warning threshold.
   - Added `tags` and `exclude_tags` filters to the device list endpoint - multiple tags with wildcard support.
   - Normalised `atmosphere` to a boolean on both planet and moon responses. Moons retain the old `has_atmosphere` field as deprecated until next season.
   - Added `has_ward` field to star listings to indicate active system wards, locking down mining and event completion to the owner.
   - Fixed `stowed_in_device_code` consistency in the `device.stowed` event.
   - Flatpack system hubs can now be printed in systems that already have an active hub.
   - AMI fleet travel commands are now processed asynchronously to handle large device movements.
   - New set of prospecting achievements for star discovery milestones.
   - Released Bill from his obligations at MIRFAKA.

   Oh, it was an exit node? Huh. Dispatching replicants to investigate… o7
4. v2.5.0 restoration
