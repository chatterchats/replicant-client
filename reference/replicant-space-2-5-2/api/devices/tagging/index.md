...
crawled_at: "2026-08-25T15:34:30.710195+00:00"
---
...
Tags can be up to 32 characters long and may contain lowercase letters, numbers, hyphens, underscores, colons, and dots.
...
The [device list](../list/index.md) endpoint supports `tags` and `exclude_tags` query parameters for advanced filtering. Both accept comma-separated patterns with `*` wildcards - for example `?tags=squad2:*` returns all devices tagged with any `squad2:` prefix, and `?exclude_tags=*:miners` omits devices with any tag ending in `:miners`.
