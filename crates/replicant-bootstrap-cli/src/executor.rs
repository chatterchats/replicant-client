...
use replicant_printing::{PrintRequest, managed::{QueueOptions, fetch_blueprints, queue_prints}};
...
use crate::{
    AnyResult, Config, app_error, reservation_tag, save_mission, unique,
    model::{BootstrapMission, CarrierLoad, MissionPhase, PrintState, ReplicantIdentity, SeedFreighter},
};
...
pub async fn resolve_replicant(client: &Client, query: &str) -> AnyResult<Option<ReplicantIdentity>> {
    ...
    for handle in handles {
        ...
        if snapshot.key.id.as_str().eq_ignore_ascii_case(query)
            || snapshot.name.as_deref().is_some_and(|name| name.eq_ignore_ascii_case(query))
        {
    ...
    match matches.len() {
        ...
        0 => {}
        _ => return Err(app_error(io::ErrorKind::InvalidInput, format!("replicant name {query:?} is ambiguous; use its code"))),
    }

    let profiles = client.directory().search(&raw::replicants::ReplicantListQuery {
        cursor: None,
        limit: Some(100),
        latest: None,
        name: Some(query.to_owned()),
    }).await?;
    let exact = profiles.into_iter().filter(|profile| {
        profile.id.as_str().eq_ignore_ascii_case(query)
            || profile.name.as_deref().is_some_and(|name| name.eq_ignore_ascii_case(query))
    }).collect::<Vec<_>>();
    ...
    match discovered.len() {
        0 => Ok(None),
        1 => Ok(discovered.pop()),
        _ => Err(app_error(io::ErrorKind::InvalidInput,
...
pub async fn resolve_star(client: &Client, designation: &str) -> AnyResult<(String, String)> {
    ...
    let star = catalogue.iter().find(|star| {
        star.key.id.as_str().eq_ignore_ascii_case(designation)
    }).ok_or_else(|| app_error(io::ErrorKind::NotFound, format!("star {designation} is not in the catalogue")))?;
    let entry = star.entry_point.as_ref().ok_or_else(|| {
        app_error(io::ErrorKind::InvalidData, format!("star {designation} has no known entry point"))
    })?;
    Ok((
...
pub async fn execute(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    ...
    if matches!(mission.phase, MissionPhase::StagingAtSource | MissionPhase::StagedAtSource) {
...
pub async fn stage(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::StagedAtSource) {
        return Err(app_error(io::ErrorKind::InvalidInput,
...
pub(crate) async fn ensure_source_entry(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if !mission.source_system.is_empty() && !mission.source_entry.is_empty() {
        return Ok(());
    }
    let catalogue = client.galaxy().catalogue();
    let source = catalogue.iter()
        .filter(|star| {
        ...
        .ok_or_else(|| app_error(io::ErrorKind::InvalidInput,
            format!("source hub {} does not resolve to a catalogue system", mission.source_hub)))?;
    mission.source_system = source.key.id.as_str().to_owned();
    mission.source_entry = source.entry_point.as_ref().map(|entry| entry.id.as_str().to_owned())
        .ok_or_else(|| app_error(io::ErrorKind::InvalidData,
            format!("source system {} has no known entry point", mission.source_system)))?;
    save_mission(&config.mission_file, mission)
}

async fn resolve_required_replicants(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    ...
    if mission.operator.code == mission.explorer.code {
        return Err(app_error(io::ErrorKind::InvalidInput, "operator and explorer resolved to the same replicant"));
    }
    save_mission(&config.mission_file, mission)
}

async fn set_phase(config: &Config, mission: &mut BootstrapMission, phase: MissionPhase) -> AnyResult<()> {
...
async fn manufacture_ark(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::StagedAtSource) { return Ok(()); }
    ...
    let carrier_capacity = client.raw().blueprints().list().await?.value.blueprints.into_iter()
        .find(|item| item.device_type.as_deref() == Some(SURGE_CARRIER))
        .and_then(|item| item.attach_capacity).unwrap_or(0);
    if carrier_capacity <= 0 { return Err(app_error(io::ErrorKind::InvalidData, "Surge Carrier blueprint has no positive attach capacity")); }
    ...
    let mut used = mission.assets.values().flatten().cloned().collect::<BTreeSet<_>>();
    ...
    for (device_type, quantity) in &desired {
        if device_type == AUTOFACTORY { continue; }
    ...
    if mission.carrier_target == 0 {
        if current_carriers > 0 || !mission.carrier_loads.is_empty() {
            ...
            mission.carrier_target = current_carriers
                .saturating_add(mission.profile.dedicated_surge_carriers);
            mission.reused_carrier_target = mission.carrier_target;
            info!(existing=current_carriers, target=mission.carrier_target,
                "upgraded the staged ark with dedicated relay and beacon carriers");
        } else {
            let mut candidates = devices.iter()
                .filter(|device| eligible_idle(device, &mission.mission_tag))
                .filter(|device| is_attachment_carrier(device.device_type.as_deref()))
                .filter(|device| device.attached_devices.is_empty())
                .filter_map(|device| Some((device.device_code.clone()?, device.attach_capacity.unwrap_or(carrier_capacity))))
                .filter(|(code, _)| !used.contains(code))
                .collect::<Vec<_>>();
            candidates.sort_by(|left, right| right.1.cmp(&left.1).then_with(|| left.0.cmp(&right.0)));
            let capacities = candidates.iter().map(|(_, capacity)| *capacity).collect::<Vec<_>>();
            ...
            info!(reused=mission.reused_carrier_target, printing=print_count,
                target=mission.carrier_target, "planned minimum attachment-carrier fleet");
        }
    }
    ...
    let reusable_missing = usize::try_from(
        mission.reused_carrier_target.saturating_sub(current_carriers),
    )?;
    let carriers = mission.assets.entry(SURGE_CARRIER.into()).or_default();
    let selected = devices.iter()
        .filter(|device| eligible_idle(device, &mission.mission_tag))
    ...
    mission.print.targets.insert(SURGE_CARRIER.into(), mission.carrier_target);
    save_mission(&config.mission_file, mission)?;
    ...
    if !mission.print.queued && !mission.print.requirements.is_empty() {
        ...
        save_mission(&config.mission_file, mission)?;
        let (modular, standard): (Vec<_>, Vec<_>) = mission.print.requirements.iter()
            .filter(|(_, quantity)| **quantity > 0)
            .map(|(device_type, quantity)| PrintRequest::new(device_type.clone(), *quantity))
            .partition(|request| blueprints.get(&request.device_type).is_some_and(|blueprint| blueprint.is_modular()));
        let tags = vec![mission.mission_tag.clone(), mission.region_tag.clone()];
        if !standard.is_empty() {
            let mut options = QueueOptions::at(&mission.source_hub);
            options.tags = tags.clone(); options.wait_timeout = config.wait_timeout;
        ...
        if !modular.is_empty() {
            let mut options = QueueOptions::at(&mission.source_hub);
            options.tags = tags; options.flatpack = true; options.wait_timeout = config.wait_timeout;
...
async fn reconcile_interrupted_print_submission(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    let tagged = list_devices(client, Some(&mission.source_hub), Some(&mission.mission_tag)).await?;
    let recorded_codes = mission.assets.values().flatten().cloned().collect::<BTreeSet<_>>();
    let mut completed = BTreeMap::<String, i64>::new();
    for device in &tagged {
        let Some(code) = device.device_code.as_ref() else { continue; };
        let Some(device_type) = device.device_type.as_ref() else { continue; };
    ...
    let recorded = mission.assets.iter().map(|(device_type, codes)| {
        (device_type.clone(), i64::try_from(codes.len()).unwrap_or(i64::MAX))
    }).collect::<BTreeMap<_, _>>();
    let previous = mission.print.requirements.clone();
    let was_submission_started = mission.print.submission_started;
    mission.print.requirements = remaining_print_requirements(
        &mission.print.targets,
        &recorded,
        &completed,
        &pending,
    );
...
async fn pending_tagged_prints(client: &Client, hub: &str, mission_tag: &str) -> AnyResult<BTreeMap<String, i64>> {
    let factories = list_devices(client, Some(hub), None).await?;
    let mut pending = BTreeMap::<String, i64>::new();
    for factory in factories.iter().filter(|device| device.device_type.as_deref() == Some(AUTOFACTORY)) {
        ...
        for job in &factory.print_queue {
            let tagged = job.get("tags").and_then(Value::as_array).is_some_and(|tags| {
                tags.iter().any(|tag| tag.as_str() == Some(mission_tag))
            });
            if !tagged { continue; }
            let Some(device_type) = ["device_type", "type"].into_iter()
                .find_map(|key| job.get(key).and_then(Value::as_str)) else { continue; };
            let quantity = ["quantity", "count"].into_iter()
                .find_map(|key| job.get(key).and_then(Value::as_i64)).unwrap_or(1).max(1);
...
fn remaining_print_requirements(
    ...
    targets.iter().filter_map(|(device_type, target)| {
        let accounted = recorded.get(device_type).copied().unwrap_or(0)
...
async fn wait_for_printed_assets(client: &Client, config: &Config, mission: &BootstrapMission, desired: &BTreeMap<String, i64>) -> AnyResult<()> {
    ...
    loop {
        ...
        let complete = desired.iter().all(|(device_type, quantity)| {
            ...
            let printed = tagged.iter().filter(|device| device.device_type.as_deref() == Some(device_type)
                && device.device_code.as_ref().is_some_and(|code| !mission.assets.values().flatten().any(|existing| existing == code))).count();
            recorded.saturating_add(printed) >= usize::try_from(*quantity).unwrap_or(usize::MAX)
        });
        let loaded_carriers = mission.carrier_loads.iter()
            .map(|load| load.carrier.clone())
            .collect::<BTreeSet<_>>();
        let loaded_capacity = mission.carrier_loads.iter()
            .map(|load| load.capacity.max(0))
            .sum::<i64>();
        let hub_carriers = tagged.iter()
            .filter(|device| is_attachment_carrier(device.device_type.as_deref()))
            .filter(|device| device.device_code.as_ref().is_some_and(|code| !loaded_carriers.contains(code)))
            .collect::<Vec<_>>();
        let carrier_capacity = loaded_capacity.saturating_add(hub_carriers.iter()
        ...
        if complete && carriers_ready { return Ok(()); }
        if Instant::now() >= deadline { return Err(app_error(io::ErrorKind::TimedOut, "timed out waiting for the regional ark to finish printing")); }
        sleep(POLL_INTERVAL).await;
    }
}

async fn allocate_printed_assets(client: &Client, config: &Config, mission: &mut BootstrapMission, desired: &BTreeMap<String, i64>) -> AnyResult<()> {
    let tagged = list_devices(client, Some(&mission.source_hub), Some(&mission.mission_tag)).await?;
    let mut used = mission.assets.values().flatten().cloned().collect::<BTreeSet<_>>();
    for (device_type, quantity) in desired {
        let current = mission.assets.entry(device_type.clone()).or_default();
        let missing = usize::try_from(quantity.saturating_sub(i64::try_from(current.len())?))?;
        let selected = tagged.iter().filter(|device| device.device_type.as_deref() == Some(device_type))
            .filter_map(|device| device.device_code.clone()).filter(|code| !used.contains(code)).take(missing)
            .collect::<Vec<_>>();
        for code in selected {
            used.insert(code.clone()); current.push(code);
        }
        if current.len() < usize::try_from(*quantity)? {
            return Err(app_error(io::ErrorKind::NotFound, format!("printed {device_type} outputs are incomplete")));
        }
    }
    let loaded_carriers = mission.carrier_loads.iter()
        .map(|load| load.carrier.clone())
        .collect::<BTreeSet<_>>();
    let mut capacity = mission.carrier_loads.iter()
        .map(|load| load.capacity.max(0))
        .sum::<i64>();
    let carriers = mission.assets.entry(SURGE_CARRIER.into()).or_default();
    capacity = capacity.saturating_add(tagged.iter()
        .filter(|device| device.device_code.as_ref().is_some_and(|code| {
            carriers.contains(code) && !loaded_carriers.contains(code)
        }))
        .map(|device| device.attach_capacity.unwrap_or(0).max(0))
        .sum::<i64>());
    for device in tagged.iter().filter(|device| is_attachment_carrier(device.device_type.as_deref())) {
        if capacity >= attachment_slots(desired)
            && carriers.len() >= usize::try_from(mission.carrier_target)?
        {
            break;
        }
        let Some(code) = device.device_code.clone() else { continue; };
    ...
    if capacity < attachment_slots(desired)
        || carriers.len() < usize::try_from(mission.carrier_target)?
    {
        return Err(app_error(io::ErrorKind::NotFound,
...
async fn claim_recorded_assets(client: &Client, mission: &BootstrapMission) -> AnyResult<()> {
    let owner = mission.operator.is_resolved().then_some(mission.operator.code.as_str());
    for code in mission.assets.values().flatten() {
        ensure_claim(client, code, owner, &[mission.mission_tag.clone(), mission.region_tag.clone()]).await?;
    }
    Ok(())
}

async fn claim_staged_ark_for_operator(client: &Client, mission: &BootstrapMission) -> AnyResult<()> {
    let assembled_at_source = !mission.carrier_loads.is_empty() && matches!(mission.phase,
...
async fn load_ark(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::StagedAtSource) { return Ok(()); }
    ...
    if mission.seed_freighters.is_empty() {
        ...
        if freighters.len() < SEED_RESOURCES.len() { return Err(app_error(io::ErrorKind::InvalidData, "ark has fewer than six Cargo Freighters")); }
        ensure_seed_inventory(client, mission).await?;
        mission.seed_freighters = freighters.into_iter().zip(SEED_RESOURCES).map(|(code, resource)| SeedFreighter {
            code, resource: resource.into(), quantity: mission.seed_quantity,
        }).collect();
        save_mission(&config.mission_file, mission)?;
    }
    let collect_results = join_all(mission.seed_freighters.iter().map(|seed| collect_resource(client, &seed.code, &seed.resource, seed.quantity))).await;
    finish_all(collect_results)?;

    append_missing_carrier_loads(client, config, mission).await?;
    let attach_results = join_all(mission.carrier_loads.iter().map(|load| attach_devices(client, &load.carrier, &load.devices))).await;
...
async fn append_missing_carrier_loads(
    ...
    let assigned = mission.carrier_loads.iter()
        .flat_map(|load| load.devices.iter().cloned())
        .collect::<BTreeSet<_>>();
    let mut unassigned = mission.assets.iter()
    ...
    let used_carriers = mission.carrier_loads.iter()
        .map(|load| load.carrier.clone())
        .collect::<BTreeSet<_>>();
    let mut carriers = Vec::new();
    for carrier in mission.assets.get(SURGE_CARRIER).cloned().unwrap_or_default()
        .into_iter().filter(|carrier| !used_carriers.contains(carrier))
    {
        let capacity = client.raw().devices().get(&carrier).await?.value.attach_capacity.unwrap_or(0);
    ...
    let expansion_relays = mission.assets.get(FTL_RELAY).into_iter().flatten()
        .filter(|code| code.as_str() != root_relay && unassigned.contains(*code))
        .cloned().collect::<Vec<_>>();
    let beacons = mission.assets.get(replicant_bootstrap_planner::FTL_BEACON)
        .into_iter().flatten().filter(|code| unassigned.contains(*code))
        .cloned().collect::<Vec<_>>();
    let mut reserved_groups = expansion_relays.chunks(9).map(|chunk| chunk.to_vec())
    ...
    for group in reserved_groups {
        ...
        let Some(index) = carriers.iter().position(|(_, capacity)| *capacity >= required) else {
            return Err(app_error(io::ErrorKind::InvalidData,
                format!("no unused carrier can hold a reserved {required}-device relay/beacon load")));
        };
        let (carrier, capacity) = carriers.remove(index);
        for code in &group { unassigned.remove(code); }
        mission.carrier_loads.push(CarrierLoad { carrier, capacity, devices: group });
    }
    ...
    for (carrier, capacity) in carriers {
        ...
        cursor += take;
        mission.carrier_loads.push(CarrierLoad { carrier, capacity, devices });
        if cursor == payload.len() { break; }
    }
    if cursor != payload.len() {
        return Err(app_error(io::ErrorKind::InvalidData,
            format!("carrier capacity covers {cursor} of {} remaining payload devices", payload.len())));
    }
    save_mission(&config.mission_file, mission)
}

async fn stage_at_source_entry(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    ...
    let devices = mission.carrier_loads.iter().map(|load| load.carrier.clone())
        .chain(mission.assets.get(CARGO_FREIGHTER).into_iter().flatten().cloned())
        .collect::<Vec<_>>();
    ...
    finish_all(join_all(devices.iter().map(|code| {
        start_device_travel(client, code, &mission.source_entry)
    })).await)?;
    finish_all(join_all(devices.iter().map(|code| {
...
async fn dispatch_to_landing(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::Outbound) { return Ok(()); }
    ...
    let (device_waits, operator_wait, explorer_wait) = tokio::join!(
        ...
        wait_replicant_at(client, &mission.explorer.code, &mission.landing_entry, config.wait_timeout),
    );
    device_waits?; operator_wait?; explorer_wait
}

async fn quick_scout(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    ...
    let mut candidates =
        belt_candidates(client, &mission.landing_star, &mission.quick_scouted_systems).await?;
...
fn nearest_neighbor_system_route(
    ...
    while !remaining.is_empty() && route.len() < system_limit {
        let Some((index, _)) = remaining
            ...
            .min_by(|(left_index, left_distance), (right_index, right_distance)| {
                left_distance
    ...
    loop {
        ...
        if snapshot.travel.is_none()
            && snapshot.location.as_ref().is_some_and(|location| {
                designation_in_system(location.id.as_str(), destination)
            })
        {
            return Ok(());
        }
        ...
        if let Some(travel) = &snapshot.travel {
            ...
            info!(
                replicant = code,
                destination,
                "quick scout departing for system"
            );
...
async fn scan_system(client: &Client, replicant_code: &str, system: &str) -> AnyResult<()> {
    ...
    if explored {
        info!(
            ...
            system,
            "quick scout system was already scanned"
        );
    } else {
        info!(
            replicant = replicant_code,
...
async fn recover_legacy_quick_survey_fleet(
    ...
    let Some(drones) = mission.assets.get(SURVEY_DRONE).map(|codes| {
        codes
            .iter()
            .rev()
            .take(3)
...
async fn establish_capital(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::EstablishingCapital) { return Ok(()); }
    ...
    let maintenance = mission.assets.get(MAINTENANCE_DRONE)
        .map(|codes| codes.iter().rev().take(usize::try_from(mission.profile.hub_maintenance_drones).unwrap_or(0)).cloned().collect::<Vec<_>>())
        .filter(|codes| codes.len() == usize::try_from(mission.profile.hub_maintenance_drones).unwrap_or(usize::MAX))
        .ok_or_else(|| app_error(io::ErrorKind::InvalidData, "ark has too few capital maintenance drones"))?;
    let infrastructure = vec![relay.clone()];
    let infrastructure_carrier = mission.carrier_loads.iter().find(|load| load.devices.contains(&relay))
        .map(|load| load.carrier.clone()).ok_or_else(|| app_error(io::ErrorKind::InvalidData, "FTL Relay is not assigned to a carrier"))?;
    ...
    let (infra_wait, carriers_wait, freighters_wait, operator_wait, explorer_wait) = tokio::join!(
        ...
        async { finish_all(join_all(freighters.iter().map(|code| wait_device_at(client, code, &capital_belt, config.wait_timeout))).await) },
        wait_replicant_at(client, &mission.operator.code, &capital_belt, config.wait_timeout),
        wait_replicant_at(client, &mission.explorer.code, &capital_belt, config.wait_timeout),
    );
    infra_wait?; carriers_wait?; freighters_wait?; operator_wait?; explorer_wait?;
    ...
    wait_device_at(client, &infrastructure_carrier, &capital_belt, config.wait_timeout).await?;
    let detach_results = join_all(mission.carrier_loads.iter().map(|load| async {
        ...
        let attached = attached_codes(&detail).into_iter().filter(|code| load.devices.contains(code)).collect::<Vec<_>>();
        detach_devices(client, &load.carrier, &attached).await
    })).await;
    finish_all(detach_results)?;
    let unfold_results = join_all(mission.assets.get(AUTOFACTORY).into_iter().flatten().map(|code| configure_structure(client, code, false))).await;
    finish_all(unfold_results)?;
    for code in &maintenance { set_patrol(client, code).await?; }
    let deposit_results = join_all(freighters.iter().map(|code| deposit_all(client, code))).await;
    finish_all(deposit_results)
}

async fn establish_initial_mine(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::InitialMining) { return Ok(()); }
    set_phase(config, mission, MissionPhase::InitialMining).await?;
    execute_mining(client, &MiningExpansionRequest {
        systems: vec![required_field(&mission.capital_system, "capital system")?.to_owned()],
        replicant: mission.operator.code.clone(), hub: required_field(&mission.capital_belt, "capital belt")?.to_owned(),
        mission_file: mission.children.initial_mining.clone(), wait_timeout: config.wait_timeout,
        max_concurrency: mission.max_concurrency,
    }).await?;
    Ok(())
}

async fn survey_region(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::SurveyingRegion) { return Ok(()); }
    set_phase(config, mission, MissionPhase::SurveyingRegion).await?;
    let controller = mission.assets.get(SURVEY_CONTROLLER).and_then(|codes| codes.last()).cloned();
    let drones = mission.assets.get(SURVEY_DRONE).map(|codes| codes.iter().rev().take(3).cloned().collect::<Vec<_>>());
    let capital_system = required_field(&mission.capital_system, "capital system")?.to_owned();
    let report = execute_survey(client, &SurveyRequest {
        replicant: mission.explorer.code.clone(), vessel: mission.explorer.vessel.clone(), center: capital_system.clone(),
        radius_ly: mission.survey_radius_ly, system_limit: 100, star_detail_concurrency: 16,
        mission_file: mission.children.survey.clone(), controller, drones, include_explored: false,
        travel_timeout: config.wait_timeout, survey_timeout: config.wait_timeout,
    }).await?;
    ...
    mission.selected_belts = select_dense_belts(&candidates, required_field(&mission.capital_belt, "capital belt")?, mission.minimum_sites, mission.maximum_sites)?;
    save_mission(&config.mission_file, mission)
}

async fn expand_relays(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::ExpandingRelays) { return Ok(()); }
    ...
    let targets = mission.selected_belts.iter().filter(|belt| belt.system != capital).map(|belt| belt.system.clone()).collect::<Vec<_>>();
    if !targets.is_empty() {
        execute_relays(client, &RelayExpansionRequest {
            replicant: mission.operator.code.clone(), hub: required_field(&mission.capital_belt, "capital belt")?.to_owned(),
            targets, mission_file: mission.children.relays.clone(), max_hop_ly: 7.499, wait_timeout: config.wait_timeout,
        }).await?;
    }
    Ok(())
}

async fn expand_mining(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    if phase_after(mission.phase, MissionPhase::ExpandingMining) { return Ok(()); }
    set_phase(config, mission, MissionPhase::ExpandingMining).await?;
    let capital = required_field(&mission.capital_system, "capital system")?;
    let systems = mission.selected_belts.iter().filter(|belt| belt.system != capital).map(|belt| belt.system.clone()).collect::<Vec<_>>();
    if !systems.is_empty() {
        execute_mining(client, &MiningExpansionRequest {
            systems, replicant: mission.operator.code.clone(), hub: required_field(&mission.capital_belt, "capital belt")?.to_owned(),
            mission_file: mission.children.mining.clone(), wait_timeout: config.wait_timeout,
            max_concurrency: mission.max_concurrency,
        }).await?;
    }
    Ok(())
...
async fn cleanup(client: &Client, config: &Config, mission: &mut BootstrapMission) -> AnyResult<()> {
    set_phase(config, mission, MissionPhase::CleaningUp).await?;
    let codes = mission.assets.values().flatten().cloned().collect::<Vec<_>>();
    let results = join_all(codes.iter().map(|code| remove_tag(client, code, &mission.mission_tag))).await;
    for result in results { if let Err(error) = result { warn!(%error, "could not remove one bootstrap reservation tag"); mission.warnings.push(error.to_string()); } }
    mission.phase = if mission.warnings.is_empty() { MissionPhase::Completed } else { MissionPhase::CompletedWithWarnings };
    save_mission(&config.mission_file, mission)?;
...
const fn phase_rank(phase: MissionPhase) -> u8 {
    match phase {
        MissionPhase::Planned => 0, MissionPhase::ManufacturingArk => 1, MissionPhase::LoadingArk => 2,
        MissionPhase::StagingAtSource => 3, MissionPhase::StagedAtSource => 4,
        MissionPhase::Outbound => 5, MissionPhase::QuickScouting => 6, MissionPhase::EstablishingCapital => 7,
        MissionPhase::InitialMining => 8, MissionPhase::SurveyingRegion => 9, MissionPhase::ExpandingRelays => 10,
        MissionPhase::ExpandingMining => 11, MissionPhase::CleaningUp => 12,
        MissionPhase::Completed | MissionPhase::CompletedWithWarnings => 13,
    }
}

async fn list_devices(client: &Client, location: Option<&str>, tag: Option<&str>) -> AnyResult<Vec<raw::devices::DeviceStatus>> {
    let mut cursor = None; let mut devices = Vec::new();
    for _ in 0..200 {
        let response = client.raw().devices().list(&raw::devices::DeviceListQuery {
            replicant_code: None, device_type: None, tag: tag.map(str::to_owned), untagged: None,
            location: location.map(str::to_owned), cursor, limit: Some(50),
        }).await?.value;
        devices.extend(response.devices);
        let Some(next) = response.next_cursor else { return Ok(devices); }; cursor = Some(next);
    }
    Err(app_error(io::ErrorKind::InvalidData, "device listing exceeded 200 pages"))
}

fn eligible_idle(device: &raw::devices::DeviceStatus, mission_tag: &str) -> bool {
    device.travel.is_none() && device.controller_device_code.is_none() && device.attached_to_device_code.is_none()
        && device.stowed_in_device_code.is_none() && device.hosting_replicant.is_none()
        && device.tags.iter().all(|tag| tag == mission_tag || !reservation_tag(tag))
        && device.status.as_deref().is_none_or(|status| matches!(status, "idle" | "inactive" | "deactivated" | "compacted"))
}

fn is_attachment_carrier(device_type: Option<&str>) -> bool {
    matches!(device_type, Some(SURGE_CARRIER | "surge_platform" | MOBILE_FLEET))
}

async fn ensure_claim(client: &Client, code: &str, owner: Option<&str>, tags: &[String]) -> AnyResult<()> {
    let handle = client.devices().get(code).await?;
    let snapshot = handle.snapshot().await?;
    let missing = tags.iter().filter(|tag| !snapshot.tags.iter().any(|existing| existing == *tag)).cloned().collect::<Vec<_>>();
    if !missing.is_empty() {
        ensure_operation(&handle.configure(raw::devices::DeviceConfiguration { add_tags: Some(missing), remove_tags: None, tags: None }).await?).await?;
    }
    let snapshot = handle.refresh().await?.snapshot().await?;
    if let Some(owner) = owner
        && snapshot.relationships.assigned_replicant.as_ref().map(|value| value.id.as_str()) != Some(owner)
    {
...
async fn remove_tag(client: &Client, code: &str, tag: &str) -> AnyResult<()> {
    ...
    if snapshot.tags.iter().any(|existing| existing == tag) {
        ensure_operation(&handle.configure(raw::devices::DeviceConfiguration { add_tags: None, remove_tags: Some(vec![tag.to_owned()]), tags: None }).await?).await?;
    }
    Ok(())
}

async fn ensure_seed_inventory(client: &Client, mission: &BootstrapMission) -> AnyResult<()> {
    let response = client.raw().inventory().list(&raw::inventory::AccountInventoryQuery {
        location: Some(mission.source_hub.clone()), cursor: None, limit: Some(50),
    }).await?.value;
    let inventory = response.locations.into_iter().find(|location| location.location.as_deref() == Some(&mission.source_hub))
        .map(|location| location.items.into_iter().filter_map(|item| Some((item.resource_type?, item.quantity.unwrap_or(0)))).collect::<BTreeMap<_,_>>()).unwrap_or_default();
    for resource in SEED_RESOURCES {
        let available = inventory.get(resource).copied().unwrap_or(0);
        if available < mission.seed_quantity { return Err(app_error(io::ErrorKind::Other, format!("source hub needs {} {resource} for seed cargo but has {available}", mission.seed_quantity))); }
    }
    Ok(())
}

async fn collect_resource(client: &Client, code: &str, resource: &str, quantity: i64) -> AnyResult<()> {
    ...
    if have >= quantity { return Ok(()); }
    if !cargo.is_empty() {
        deposit_all(client, code).await?;
    }
    let resources = [(resource.to_owned(), Value::from(quantity))].into_iter().collect();
    ensure_operation(&client.devices().get(code).await?.command(raw::devices::DeviceCommand::CollectResources { resources }).await?).await
}

async fn deposit_all(client: &Client, code: &str) -> AnyResult<()> {
    let detail = client.raw().devices().get(code).await?.value;
    if cargo_map(&detail).is_empty() { return Ok(()); }
    ensure_operation(&client.devices().get(code).await?.command(raw::devices::DeviceCommand::DepositResources { resources: None }).await?).await
}

fn cargo_map(device: &raw::devices::DeviceStatus) -> BTreeMap<String, i64> {
    device.cargo.iter().filter_map(|item| Some((item.resource_type.clone()?, item.quantity.unwrap_or(0)))).collect()
}
...
async fn attach_devices(client: &Client, carrier: &str, devices: &[String]) -> AnyResult<()> {
    ...
    let missing = devices.iter().filter(|code| !attached.contains(*code)).cloned().collect::<Vec<_>>();
    if missing.is_empty() { return Ok(()); }
    ensure_operation(&client.devices().get(carrier).await?.attach(targets(&missing)).await?).await
}
...
async fn detach_devices(client: &Client, carrier: &str, devices: &[String]) -> AnyResult<()> {
    ...
    let detail = client.raw().devices().get(carrier).await?.value;
    let attached = attached_codes(&detail);
    let present = devices.iter().filter(|code| attached.contains(*code)).cloned().collect::<Vec<_>>();
    if present.is_empty() { return Ok(()); }
    ensure_operation(&client.devices().get(carrier).await?.command(raw::devices::DeviceCommand::Detach(targets(&present))).await?).await
}

fn targets(devices: &[String]) -> raw::devices::TargetsCommand {
    raw::devices::TargetsCommand { device: None, devices: Some(Value::Array(devices.iter().cloned().map(Value::String).collect())), target: None, targets: None }
}

fn attached_codes(device: &raw::devices::DeviceStatus) -> BTreeSet<String> {
    device.attached_devices.iter().filter_map(reference_code).collect()
}
...
async fn start_device_travel(client: &Client, code: &str, destination: &str) -> AnyResult<()> {
    let detail = client.raw().devices().get(code).await?.value;
    if detail.travel.is_none() && detail.location.as_deref() == Some(destination) { return Ok(()); }
    if let Some(travel) = &detail.travel {
        let planned = travel.final_destination.as_deref().or(travel.destination.as_deref());
        if planned == Some(destination) { return Ok(()); }
        return Err(app_error(io::ErrorKind::Other, format!("device {code} is travelling to {planned:?}, not {destination}")));
    }
    ensure_operation(&client.devices().get(code).await?.command(raw::devices::DeviceCommand::Travel { destination: destination.into(), dry_run: None, via: None }).await?).await
}

async fn wait_device_at(client: &Client, code: &str, destination: &str, timeout: Duration) -> AnyResult<()> {
    let deadline = Instant::now() + timeout;
    loop {
        let detail = client.raw().devices().get(code).await?.value;
        if detail.travel.is_none() && detail.location.as_deref() == Some(destination) { return Ok(()); }
        if Instant::now() >= deadline { return Err(app_error(io::ErrorKind::TimedOut, format!("timed out waiting for device {code} at {destination}"))); }
        sleep(POLL_INTERVAL).await;
    }
}
...
async fn start_replicant_travel(client: &Client, code: &str, destination: &str) -> AnyResult<()> {
    let handle = client.replicants().get_owned(code).await?;
    let snapshot = handle.snapshot().await?;
    if snapshot.travel.is_none() && snapshot.location.as_ref().is_some_and(|location| location.id.as_str() == destination) { return Ok(()); }
    if let Some(travel) = &snapshot.travel {
        let planned = travel.final_destination.as_ref().or(travel.destination.as_ref()).map(|location| location.id.as_str());
        if planned == Some(destination) { return Ok(()); }
        return Err(app_error(io::ErrorKind::Other, format!("replicant {code} is travelling to {planned:?}, not {destination}")));
    }
    ensure_operation(&handle.travel().to(destination).depart().await?).await
}

async fn wait_replicant_at(client: &Client, code: &str, destination: &str, timeout: Duration) -> AnyResult<()> {
    let deadline = Instant::now() + timeout;
    loop {
        let snapshot = client.replicants().get_owned(code).await?.snapshot().await?;
        if snapshot.travel.is_none() && snapshot.location.as_ref().is_some_and(|location| location.id.as_str() == destination) { return Ok(()); }
        if snapshot.travel.is_none() {
            start_replicant_travel(client, code, destination).await?;
        }
        if Instant::now() >= deadline { return Err(app_error(io::ErrorKind::TimedOut, format!("timed out waiting for replicant {code} at {destination}"))); }
        sleep(POLL_INTERVAL).await;
    }
}
...
async fn configure_structure(client: &Client, code: &str, set_entry: bool) -> AnyResult<()> {
    ...
    if detail.available_commands.iter().any(|command| command == "unfurl") {
        ensure_operation(&client.devices().get(code).await?.unfurl().await?).await?;
        detail = client.raw().devices().get(code).await?.value;
    }
    if detail.status.as_deref() != Some("active") && detail.available_commands.iter().any(|command| command == "activate") {
        ensure_operation(&client.devices().get(code).await?.activate().await?).await?;
        detail = client.raw().devices().get(code).await?.value;
    }
    if set_entry && detail.available_commands.iter().any(|command| command == "set_entry_point") {
        ensure_operation(&client.devices().get(code).await?.command(raw::devices::DeviceCommand::SetEntryPoint).await?).await?;
    }
    Ok(())
}
...
async fn set_patrol(client: &Client, code: &str) -> AnyResult<()> {
    let detail = client.raw().devices().get(code).await?.value;
    let current = detail.ami_directive.as_ref().and_then(|value| value.get("directive")).and_then(Value::as_str);
    if current != Some("patrol") {
        ensure_operation(&client.devices().get(code).await?.command(raw::devices::DeviceCommand::SetDirective { directive: "patrol".into(), configuration: None, notify: None }).await?).await?;
    }
    Ok(())
}

async fn belt_candidates(client: &Client, center: &str, systems: &[String]) -> AnyResult<Vec<BeltCandidate>> {
    let catalogue = client.galaxy().catalogue();
    let center_position = catalogue.iter().find(|star| star.key.id.as_str() == center).and_then(|star| star.position)
        .ok_or_else(|| app_error(io::ErrorKind::InvalidData, format!("{center} has no catalogue position")))?;
    let positions = catalogue.iter().filter_map(|star| Some((star.key.id.as_str().to_owned(), star.position?))).collect::<BTreeMap<_,_>>();
    let mut result = Vec::new();
    for system in systems {
        let location = client.locations().get(system).await?;
        let position = positions.get(system).copied().ok_or_else(|| app_error(io::ErrorKind::InvalidData, format!("{system} has no catalogue position")))?;
        let distance = distance(center_position, position);
        result.extend(belts_from_location(&location).into_iter().map(|(designation, density)| BeltCandidate {
            system: system.clone(), designation, density, distance_from_capital_ly: distance,
        }));
    }
    Ok(result)
}
...
fn belts_from_location(location: &Location) -> Vec<(String, String)> {
    let Some(asteroid_belt) = location.unknown.get("asteroid_belt") else { return Vec::new(); };
    asteroid_belt.get("belts").and_then(Value::as_array).map(Vec::as_slice).unwrap_or_else(|| std::slice::from_ref(asteroid_belt))
        .iter().filter_map(|value| { let object = value.as_object()?; Some((object.get("designation")?.as_str()?.to_owned(), object.get("density").and_then(Value::as_str).unwrap_or("unknown").to_owned())) }).collect()
}

fn density_rank(value: &str) -> u8 { match value.to_ascii_lowercase().as_str() { "dense" => 3, "moderate" => 2, "sparse" => 1, _ => 0 } }
...
async fn ensure_operation(operation: &Operation) -> AnyResult<()> {
    let outcome = operation.outcome().await?;
    if matches!(outcome.status, OperationStatus::Cancelled | OperationStatus::Rejected | OperationStatus::Failed) {
        return Err(app_error(io::ErrorKind::Other, format!("operation {} ended as {:?}: {:?}", operation.id().as_str(), outcome.status, outcome.response)));
    }
    Ok(())
}

fn finish_all(results: Vec<AnyResult<()>>) -> AnyResult<()> { for result in results { result?; } Ok(()) }

fn first_asset(mission: &BootstrapMission, device_type: &str) -> AnyResult<String> {
    mission.assets.get(device_type).and_then(|codes| codes.first()).cloned()
        .ok_or_else(|| app_error(io::ErrorKind::InvalidData, format!("ark has no {device_type}")))
}

fn required_field<'a>(value: &'a Option<String>, name: &str) -> AnyResult<&'a str> {
    value.as_deref().ok_or_else(|| app_error(io::ErrorKind::InvalidData, format!("mission has no {name}")))
}
...
mod tests {
    ...
    fn phase_order_is_monotonic() {
        assert!(phase_after(MissionPhase::SurveyingRegion, MissionPhase::InitialMining));
        assert!(phase_after(MissionPhase::StagedAtSource, MissionPhase::LoadingArk));
        assert!(!phase_after(MissionPhase::StagedAtSource, MissionPhase::Outbound));
        assert!(!phase_after(MissionPhase::Outbound, MissionPhase::Outbound));
    }
    #[test]
    fn density_order_prefers_dense() { assert!(density_rank("dense") > density_rank("moderate")); }

    #[test]
    fn quick_scout_route_starts_at_center_and_uses_nearest_neighbors() {
        let route = nearest_neighbor_system_route(
            "CENTER".into(),
            GalacticPosition { x: 0.0, y: 0.0, z: 0.0 },
            vec![
                ("FAR".into(), GalacticPosition { x: 3.0, y: 0.0, z: 0.0 }),
                ("NEAR".into(), GalacticPosition { x: 1.0, y: 0.0, z: 0.0 }),
                ("MIDDLE".into(), GalacticPosition { x: 2.0, y: 0.0, z: 0.0 }),
            ],
            4,
        );
    ...
    fn interrupted_print_reconciliation_does_not_duplicate_accounted_work() {
        let targets = [("autofactory".into(), 1), ("survey_drone".into(), 4)].into_iter().collect();
    ...
    fn staged_manifest_upgrade_only_queues_the_new_reserve() {
        ...
        let recorded = [
            ("ftl_relay".into(), 1),
            ("surge_carrier".into(), 9),
        ]
        .into_iter()
        .collect();

        let remaining = remaining_print_requirements(
            &targets,
            &recorded,
            &BTreeMap::new(),
            &BTreeMap::new(),
    ...
    fn old_missions_drop_system_hub_printing() {
        ...
        assert_eq!(print.targets, [("autofactory".into(), 6)].into_iter().collect());
        assert_eq!(print.requirements, [("autofactory".into(), 6)].into_iter().collect());
        assert!(!remove_system_hub_request(&mut print));
    }
}
