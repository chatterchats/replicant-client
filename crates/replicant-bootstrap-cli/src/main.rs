...
async fn main() -> AnyResult<()> {
    ...
    if config.command == Command::Run && !config.mission_file.exists() {
        return Err(app_error(
            ...
            format!(
                "no mission at {}; create it with `replicant-bootstrap plan`",
...
async fn create_plan(client: &Client, config: &Config) -> AnyResult<()> {
    ...
    if operator.code == explorer.code {
        return Err(app_error(
            io::ErrorKind::InvalidInput,
...
pub(crate) fn load_mission(path: &Path) -> AnyResult<BootstrapMission> {
    let mission: BootstrapMission = serde_json::from_slice(&fs::read(path)?)?;
...
fn init_logging(config: &Config) -> AnyResult<()> {
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("warn,replicant_bootstrap=info,replicant_client::ops=info")
    });
