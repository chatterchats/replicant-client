...
`replicantd` is the long-running local Replicant application process. It owns one managed client, one long-lived history database, one workflow database, one isolated telemetry database, and one workflow supervisor.
...
sqlite3 replicant-telemetry.sqlite ".backup 'backup/replicant-telemetry.sqlite'"
...
sqlite3 backup/replicant-telemetry.sqlite "PRAGMA integrity_check;"
```

The history and telemetry database names are derived from `REPLICANT_DB` by default (`replicant-client.sqlite` -> `replicant-history.sqlite` and `replicant-telemetry.sqlite`; custom names use `.history.sqlite` and `.telemetry.sqlite` suffixes). Keep the managed, history, runtime, and telemetry databases as one dated backup set. Do not copy a live `.sqlite` file by itself because WAL contents may be omitted. To restore, stop the daemon, preserve the failed files for diagnosis, replace all four databases from the same backup set, verify `PRAGMA integrity_check` returns `ok`, then restart and inspect the startup reconciliation summary.
