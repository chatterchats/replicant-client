...
pub fn look_at(ex: f32, ey: f32, ez: f32, tx: f32, ty: f32, tz: f32) -> Mat4 {
    ...
    [
        xx, yx, zx, 0.0, xy, yy, zy, 0.0, xz, yz, zz, 0.0, -(xx * ex + xy * ey + xz * ez), -(yx * ex + yy * ey + yz * ez),
        -(zx * ex + zy * ey + zz * ez), 1.0,
    ]
}
...
pub fn perspective(fov_y: f32, aspect: f32, near: f32, far: f32) -> Mat4 {
    ...
    let nf = 1.0 / (near - far);
    [f / aspect, 0.0, 0.0, 0.0, 0.0, f, 0.0, 0.0, 0.0, 0.0, (near + far) * nf, -1.0, 0.0, 0.0, 2.0 * near * far * nf, 0.0]
}
...
pub fn build_mvp(theta: f32, phi: f32, distance: f32, tx: f32, ty: f32, tz: f32, aspect: f32) -> Mat4 {
    let cos_p = phi.cos();
    let sin_p = phi.sin();
    let cos_t = theta.cos();
    let sin_t = theta.sin();
    ...
    let proj = perspective(std::f32::consts::FRAC_PI_3, aspect, distance * 0.001, distance * 12.0 + 50.0);
    let view = look_at(eye_x, eye_y, eye_z, tx, ty, tz);
    multiply_mat4(&proj, &view)
}

pub fn project_point(mvp: &Mat4, x: f32, y: f32, z: f32, css_w: f32, css_h: f32) -> Option<(f32, f32)> {
