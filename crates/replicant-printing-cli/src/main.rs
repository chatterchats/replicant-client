...
impl Config {
    fn from_args_and_env() -> AnyResult<Self> {
        let mut raw_arguments = env::args().skip(1).collect::<Vec<_>>();
...
Usage:\n  replicant-printing [queue] --print QUANTITY DEVICE_TYPE [OPTIONS]\n  replicant-printing clear [--system SYSTEM] [OPTIONS]\n  replicant-printing status [--system SYSTEM] [--print N DEVICE_TYPE] [OPTIONS]\n\n\
