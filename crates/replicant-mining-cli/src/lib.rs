...
Usage:\n  replicant-mining plan [SYSTEM ...] [OPTIONS]\n  replicant-mining run [OPTIONS]\n  replicant-mining status [OPTIONS]\n\n\
...
pub async fn run_cli() -> AnyResult<()> {
    ...
    if config.command == Command::Run && !config.plan_path.exists() {
        return Err(app_error(
            ...
            format!(
                "no mining mission exists at {}; create one with `replicant-mining plan ...`",
...
fn init_logging(config: &Config) -> AnyResult<()> {
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("warn,replicant_mining_cli=info,replicant_client::ops=info")
    });
...
fn print_plan(mission: &MiningMission, plan_path: &Path, json: bool) -> AnyResult<()> {
    ...
    println!("Start or continue with: replicant-mining run");
    Ok(())
}
